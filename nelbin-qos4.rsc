# apr/27/2017 22:37:10 by RouterOS 6.38.5
...
add kind=pcq name=PCQ-IN pcq-classifier=dst-address pcq-dst-address6-mask=64 \
    pcq-src-address6-mask=64 pcq-total-limit=750000KiB
/queue tree
add max-limit=9500k name=IN parent=global queue=PCQ-IN
add max-limit=750k name=OUT parent=global queue=PCQ-OUT
add limit-at=500k max-limit=9500k name=4.Browsing packet-mark=browsing-in \
    parent=IN priority=4 queue=PCQ-IN
add limit-at=500k max-limit=9500k name=5.Video packet-mark=video-in parent=IN \
    priority=5 queue=PCQ-IN
add limit-at=500k max-limit=9500k name=2.Games packet-mark=games-in parent=IN \
    priority=2 queue=PCQ-IN
add limit-at=100k max-limit=9500k name=7.Downloads packet-mark=downloads-in \
    parent=IN priority=7 queue=PCQ-IN
add limit-at=50k max-limit=750k name=7.Uploads-U packet-mark=uploads-out \
    parent=OUT priority=7 queue=PCQ-OUT
add limit-at=50k max-limit=750k name=2.Games-U packet-mark=games-out parent=\
    OUT priority=2 queue=PCQ-OUT
add limit-at=50k max-limit=750k name=4.Browsing-U packet-mark=http-out \
    parent=OUT priority=4 queue=PCQ-OUT
add limit-at=50k max-limit=750k name=5.Video-U packet-mark=video-out parent=\
    OUT priority=5 queue=PCQ-OUT
...
add limit-at=50k max-limit=750k name=8.Bulk-U packet-mark=bulk-out parent=OUT \
    queue=PCQ-OUT
add limit-at=500k max-limit=9500k name=3.VOIP packet-mark=voip-in parent=IN \
    priority=3 queue=PCQ-IN
add limit-at=50k max-limit=750k name=3.VOIP-U packet-mark=voip-out parent=OUT \
    priority=3 queue=PCQ-OUT
add limit-at=100k max-limit=9500k name=1.Service packet-mark=service-in \
    parent=IN priority=1 queue=PCQ-IN
add limit-at=50k max-limit=750k name=1.Service-U packet-mark=service-out \
    parent=OUT priority=1 queue=PCQ-OUT
add max-limit=750k name=1.ICMP packet-mark=ping-out parent=OUT priority=1 \
    queue=PCQ-OUT
/ip address
...
/ip firewall mangle
add action=mark-connection chain=forward comment="#####  ICMP" \
    new-connection-mark=icmp passthrough=yes protocol=icmp
add action=mark-packet chain=forward comment=ping connection-mark=icmp \
    new-packet-mark=ping-out passthrough=yes src-address=192.168.1.0/24
