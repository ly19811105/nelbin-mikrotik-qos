# apr/28/2017 18:50:33 by RouterOS 6.38.5
...
set 0 kind=bfifo
/queue tree
add max-limit=10M name=IN parent=lan queue=default
add max-limit=750k name=OUT parent=wan queue=default
add limit-at=500k max-limit=10M name=4.Browsing packet-mark=browsing parent=\
    IN priority=4 queue=default
add limit-at=1M max-limit=10M name=5.Video packet-mark=video parent=IN \
    priority=5 queue=default
add limit-at=500k max-limit=10M name=2.Games packet-mark=games parent=IN \
    priority=2 queue=default
add max-limit=10M name=6.Downloads packet-mark=transfer parent=IN priority=6 \
    queue=default
add max-limit=750k name=6.Uploads-o packet-mark=transfer parent=OUT priority=\
    6 queue=default
add limit-at=50k max-limit=750k name=2.Games-o packet-mark=games parent=OUT \
    priority=2 queue=default
add limit-at=50k max-limit=750k name=4.Browsing-o packet-mark=browsing \
    parent=OUT priority=4 queue=default
add limit-at=50k max-limit=750k name=5.Video-o packet-mark=video parent=OUT \
    priority=5 queue=default
add max-limit=10M name=8.Bulk packet-mark=bulk parent=IN queue=default
add max-limit=750k name=8.Bulk-o packet-mark=bulk parent=OUT queue=default
add limit-at=500k max-limit=10M name=3.VOIP packet-mark=voip parent=IN \
    priority=3 queue=default
add limit-at=50k max-limit=750k name=3.VOIP-o packet-mark=voip parent=OUT \
    priority=3 queue=default
add limit-at=32k max-limit=10M name=1.Service packet-mark=service parent=IN \
    priority=1 queue=default
add limit-at=50k max-limit=750k name=1.Service-o packet-mark=service parent=\
...
add action=mark-connection chain=prerouting comment=browsing dst-port=80,443 \
